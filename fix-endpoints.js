...
for (const file of files) {
  ...
  for (const [plural, singular] of Object.entries(mapping)) {
    ...
    // Wait, the backend has no `/ordem`, we should probably leave it or change it to something else. But let's just do standard ones:
    content = content.replace(new RegExp(`apiBackend\\.put(.*?)<.*?>(.*?)\\('/${plural}/ordem'(.*)\\)`, 'g'), `apiBackend.put$1$2('/${singular}/atualizar'$3)`); // Maybe wrong, but backend doesn't have /ordem
  ...
  // Handle UploadController specifically
  if (file === 'UploadController.ts') {
    // maybe no change needed if it's already correct. Let's leave upload.
  }
