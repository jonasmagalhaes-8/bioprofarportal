...
import { controllerListarComite, controllerDeletarMembro } from '../controllers/MembroEquipeController';
...
export class GerenciarEquipe extends React.Component<{
  onClose: () => void;
  onEdit: (membro: MembroEquipeModel) => void;
  onAdd: () => void;
}> {
  state = {
    equipe: [] as MembroEquipeModel[],
    carregando: true,
  ...
  carregarEquipe = async () => {
    const response = await controllerListarComite();
    this.setState({
      equipe: response || [],
      carregando: false,
    });
  ...
  render() {
    ...
    const { isMobile, equipe } = this.state;
      ...
      <div style={overlayStyle}>
        <div style={responsiveModalStyle} onClick={(e) => e.stopPropagation()}>
          <div style={headerStyle}>
            <h2 style={titleStyle}>Comitê Gestor</h2>
            <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
              <button style={addButtonStyle} onClick={onAdd} title="Adicionar Membro">
                +
              </button>
          ...
          <div style={contentStyle}>
            ...
            {equipe.length === 0 && !this.state.carregando && (
