...
function initMockMembros() {
  if (mockMembros.length === 0) {
    // Definimos todos os membros uma única vez. 
    // Membros que fazem parte do Comitê Gestor têm a propriedade 'comite: true'.
    const data = [
      {
        id: '1',
      ...
      // PESQUISADORES QUE NÃO SÃO DO COMITÊ (OU CUJA FUNÇÃO PRINCIPAL É PESQUISA)
      {
        id: '9',
    ...
    mockMembros = data.map(item => {
      const m = new MembroEquipeModel();
      Object.assign(m, item);
      if (m.pesquisador === undefined) {
        m.pesquisador = !m.comite;
      }
      return m;
    });
  }
...
export class MembroEquipeController {
  ...
  async obterPorId(id: number): Promise<MembroEquipeModel | null> {
    try {
  ...
  async criar(membro: Partial<MembroEquipeModel>): Promise<MembroEquipeModel> {
    try {
      const json = await apiBackend.post('/membroequipe/salvar', membro);
      return json.data.response;
    } catch (err: any) {
      const novoMembro = new MembroEquipeModel();
      Object.assign(novoMembro, membro);
      novoMembro.id = mockMembros.length + 1;
      mockMembros.push(novoMembro);
      return novoMembro;
    }
  }

  async atualizar(id: number, membro: Partial<MembroEquipeModel>): Promise<MembroEquipeModel | null> {
    try {
      let json;
    if (membro.id) {
      ...
      const index = mockMembros.findIndex(m => m.id === id);
      if (index !== -1) {
...
export const controllerListarMembros = () => controller.listar();
export const controllerObterMembroPorId = (id: number) => controller.obterPorId(id);
export const controllerCriarMembro = (membro: any) => controller.criar(membro);
export const controllerAtualizarMembro = (id: number, membro: any) => controller.atualizar(id, membro);
export const controllerDeletarMembro = (id: number) => controller.excluir(id);

export const controllerListarComite = async () => {
  const membros = await controller.listar();
  // Filtra membros que pertencem ao comitê e ordena pela ordemComite definida
  return membros
    .filter(m => (m as any).comite)
    .sort((a, b) => ((a as any).ordemComite || 99) - ((b as any).ordemComite || 99));
};

export const controllerListarPesquisadores = async () => {
  const membros = await controller.listar();
  // Os pesquisadores são aqueles que têm a flag pesquisador como true
  return membros
    .filter(m => (m as any).pesquisador)
